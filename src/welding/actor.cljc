(ns welding.actor
  "WeldingPracticeActor — the ISCO-08 7212 community welding actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one welding operation request
  ...
  The unconditional invariant: the WeldingPracticeAdvisor can never
  directly commit a record the WeldingPracticeGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [welding.advisor :as advisor]
            [welding.governor :as governor]
            [welding.store :as store]))

(defn build-graph
  "Build a compiled WeldingPracticeActor graph. `store` implements
  `welding.store/Store`. `advisor` implements
  `welding.advisor/Advisor` (defaults to `mock-advisor`).
